...
const ansysV2Options = Type.Object(
  {
    ...
    renderManifestPath: Type.Optional(Type.String({
      minLength: 1,
      description: "Optional workspace-relative PR33 deterministic ANSYS ModelSpec render manifest. When present, admission verifies the current APDL bundle back to the exact ModelSpec fingerprint.",
    })),
  },
  { additionalProperties: false },
);
...
const solverOptions = Type.Object({
  ...
  analysisManifestPath: Type.Optional(Type.String({
    description: "Workspace-relative PR26 generated-analysis manifest path. For FEMagent-rendered OpenSees analysis bundles it must be paired with responsePlanPath.",
  })),
});
...
export default function femToolsExtension(pi: ExtensionAPI) {
  ...
  pi.registerTool({
    ...
    promptGuidelines: [
      ...
      "For a ModelSpec-rendered ANSYS model, preserve the exact solverOptions.ansysV2.renderManifestPath returned by fem_model_render_ansys or fem_earthquake_workflow_prepare. Verified admission may then report semanticEquivalence=MACHINE_PROVEN_RENDER_BINDING.",
      "For arbitrary/external APDL without a verified PR33 render manifest, confirmedBundleFingerprint proves exact bytes only and semanticEquivalence remains NOT_MACHINE_PROVEN. Preserve and report the admission binding instead of overstating equivalence.",
  ...
  pi.registerTool({
    ...
    promptGuidelines: [
      ...
      "For ANSYS V2, pass exactly the same solverOptions.modelUnits and complete solverOptions.ansysV2 object (including renderManifestPath when present) that produced READY preflight, keep loadPath omitted, and do not mutate the model bundle, render manifest, or AnalysisSpec between preflight and run.",
