...
def verify_generated_analysis_bundle(
    ...
    *,
    ...
    expected_source = build_opensees_linear_static_analysis_source(
        normalized_model,
        normalized_analysis,
    )
    ...
    for label, actual_text, expected_text, hash_key in artifact_pairs:
        ...
        if actual_text != expected_text:
            raise _artifact_error(
                f"Generated-analysis {label} differs from deterministic regeneration"
            )
...
__all__ = ["VERIFICATION_SCHEMA", "verify_generated_analysis_bundle"]
